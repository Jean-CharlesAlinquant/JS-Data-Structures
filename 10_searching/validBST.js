...
class BinarySearchTree {
    ...
    isValidBST() {
        return traverseTree(this.root, -Infinity, Infinity);
    }
}
...
function traverseTree(node, min, max) {
    ...
    console.log(`Traversing node ${node.value} with min ${min} and max ${max}`);
    if(node.value < min || node.value >= max) {
        return false;
    }
    return traverseTree(node.left, min, node.value) && traverseTree(node.right, node.value, max);
}
